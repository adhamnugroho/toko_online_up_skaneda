...
class CheckoutController extends Controller
{
    ...
    protected $sub_menu = '';
    protected $directory = 'user.checkout';
    ...
    public function proses(Request $request)
    {
        ...
        // Pengecekan kosong tidaknya variabel
        if (empty($id_user)) {

            return back()
                ->withInput()
                ->with('status', 'error')
                ->with('message', 'Kolom Pembeli Harus Diisi!');
        }
        ...
        if (empty($id_keranjang)) {
            ...
            return back()
                ->withInput()
                ->with('status', 'error')
                ->with('message', 'Kolom Keranjang Harus Diisi!');
        }
        ...
        if (empty($tanggal_transaksi)) {
            ...
            return back()
                ->withInput()
                ->with('status', 'error')
                ->with('message', 'Kolom Tanggal Transaksi Harus Diisi!');
        }
        ...
        if (empty($ekspedisi)) {
            ...
            return back()
                ->withInput()
                ->with('status', 'error')
                ->with('message', 'Kolom Ekspedisi Harus Diisi!');
        }
        ...
        if ($transaksi) {
            ...
            if ($transaksi_detail) {
                ...
                // Mengurangi stok produk, berdasarkan jumlah produk yang ada di keranjang
        ...
        // Jika berhasil menyimpan data transaksi ke database
        if ($transaksi) {
    ...
    public function complete(Request $request)
    {
        ...
        if ($get_checkout) {
            ...
            return redirect()->route('userCheckoutHistory')
                ->with('status', 'error')
                ->with('message', 'Tidak Dapat Menemukan Data!');
        }
    }
}
