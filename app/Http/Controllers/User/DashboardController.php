...
class DashboardController extends Controller
{
    ...
    public function main(Request $request)
    {
        ...
        $data['kategori_produk'] = kategoriProduk::with('product')->get();
    ...
    public function addToCart(Request $request)
    {
        ...
        // Check Dulu Dikeranjang Ada Produk Yang Sama Atau Tidak

        if (Auth::check()) {

            $check_keranjang = Keranjang::where('produk_id', $id)->where('user_id', Auth::id())->first();
            ...
            if ($check_keranjang) {
                ...
                return redirect()->route('userDashboard')
                    ->with('status', 'success')
                    ->with('message', 'Berhasil Menambahkan Ke Keranjang');
            } else {

                return redirect()->route('userDashboard')
                    ->with('status', 'error')
                    ->with('message', 'Gagal Menambahkan Ke Keranjang');
            }
        } else {
